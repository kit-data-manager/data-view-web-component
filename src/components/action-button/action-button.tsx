...
export const ActionButton = ({ action, onActionPress, noLabel }: Props) => {
  if ('url' in action) {
    return (
      <a target={action.urlTarget ?? '_blank'} href={action.url} class="action-btn-icon">
        <iconify-icon icon={action.iconName} height="1.5em"></iconify-icon>
        {noLabel ? null : <span class="subtitle">{action.label}</span>}
      </a>
    );
  }

  return (
    <button onClick={() => onActionPress(action.eventIdentifier)} class="action-btn-icon">
      <iconify-icon icon={action.iconName} height="1.5em"></iconify-icon>
      {noLabel ? null : <span class="subtitle">{action.label}</span>}
    </button>
  );
